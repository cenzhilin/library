...
export default function Index(props) {
    ...
    async function getBookshelfNeat() {
        const res = await fetch(`${state.host}/library/place`, { method: 'get' });
        const result = await res.json();

        dispatch({
            type:"allPlace",
            value:result.allPlace
        });
        
        dispatch({
            type:"noPlaceList",
            value:result.noPlace
        })
        setReCheck(1);
    }
    ...
    return (
        <Container className='bg-light'>
            <Row style={{ height: '100px' }}>
                <Col md={2}></Col>
                <Col md={8} className='text-center text-success '><h1>阶段2:图书错位({roomId}号房间)</h1></Col>
                <Col md={2}></Col>
            ...
            <Row style={{ height: '400px' }}>
                ...
                <Col md={8} className='border border-success rounded bg-white' style={{ overflow: 'auto ' }}>
                    {/* 书架 */}
                    {state.allPlace ? '已摆正' : <Row>
                        {state.noPlaceList&&state.noPlaceList.map((item,index)=>
                        <div key={index} className='bookshefl-box m-4'>
                            <Link to={`/place/${item.bookshelfId}/bookshelf/${index+1}`}>
                                <div className='bookshelf'  >
                                    {item.noPlace?<div className="pmask"></div>:<div></div>} 
                                </div></Link>
...
// state.allNeat &&
<Link to={`/`} style={{ textDecoration: 'none' }}> <div className='go bg-success' > &gt; </div></Link>} </Col>
            </Row>
            <Row style={{ height: '100px' }}>
                <Col md={2}></Col>
